...
#define TRANSITION_TIME         LV_THEME_MESHPUNK_TRANSITION_TIME
...
#if LV_THEME_MESHPUNK_TRANSITION_TIME
...
static void style_init(my_theme_t * theme)
{
    ...
    lv_style_set_transition(&theme->styles.transition_delayed,
                            &theme->trans_delayed); /*Go back to meshpunk state with delay*/

    lv_style_set_transition(&theme->styles.transition_normal, &theme->trans_normal); /*Go back to meshpunk state with delay*/
#endif
    ...
    lv_style_set_radius(&theme->styles.card, RADIUS_MESHPUNK);
    ...
    lv_style_set_radius(&theme->styles.cb_marker, RADIUS_MESHPUNK);
    ...
    lv_style_set_radius(&theme->styles.menu_section, RADIUS_MESHPUNK);
    ...
    lv_style_set_radius(&theme->styles.keyboard_button_bg,
                        theme->disp_size == DISP_SMALL ? RADIUS_MESHPUNK / 2 : RADIUS_MESHPUNK);
#endif
                                   ...
                                   const lv_font_t * font)
{
    ...
    if(!lv_theme_meshpunk_is_inited()) {
