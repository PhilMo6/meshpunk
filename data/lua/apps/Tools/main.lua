...
local function discover_tools()
    local tools = {}
    local seen = {}

    local ok1, internal_dirs = pcall(_list_dir, "/lua/apps/Tools")
    if ok1 and type(internal_dirs) == "table" then
        for _, name in ipairs(internal_dirs) do
            local entry = "/lua/apps/Tools/" .. name .. "/main.lua"
            if file_exists(entry) then
                table.insert(tools, {
                    name = name,
                    entrypoint = entry,
                    source = "internal",
                    dir = "L:/lua/apps/Tools/" .. name
                })
                seen[name] = true
                print("[Tools] Found internal: " .. name)
            end
        end
    end

    local ok2, sd_dirs = pcall(_list_dir_sd, "/meshpunk/apps/Tools")
    if ok2 and type(sd_dirs) == "table" then
        for _, name in ipairs(sd_dirs) do
            local entry = "/meshpunk/apps/Tools/" .. name .. "/main.lua"
            ...
            if ok3 and exists then
                if not seen[name] then
                    table.insert(tools, {
                        ...
                        entrypoint = entry,
                        source = "sd",
                        dir = "S:/meshpunk/apps/Tools/" .. name
                    })
                    print("[Tools] Found SD: " .. name)
                else
                    print("[Tools] SD tool " .. name .. " skipped (internal exists)")
                end
            end
        end
    end
...
root:Label{text = "Tools", align = lvgl.ALIGN.CENTER, w = 260, h = 40}
...
if #tools == 0 then
    root:Label{text = "No tools found!", align = lvgl.ALIGN.CENTER, w = 200, h = 40}
end
