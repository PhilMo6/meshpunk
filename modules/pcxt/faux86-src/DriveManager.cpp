...
{
	log(Log,"[DRIVEMANAGER] Constructed");
}
...
{
	#ifdef DEBUG_DISK
	log(Log,"[DRIVEMANAGER::insertDisk] targetDrive %u", targetDrive);
	#endif
	if (!disk) return false;

	if (!disk->isValid())	{
		#ifdef DEBUG_DISK
		log(Log,"[DRIVEMANAGER::insertDisk] Invalid Disk for targetDrive %u", targetDrive);
		#endif
		delete disk;
		return false;
	}
	...
	{ 
		...
		else
		{
			...
			#ifdef DEBUG_DISK
			log(Log,"[DRIVEMANAGER::insertDisk] Detected HardDisk Image size:%llu sects:%u heads:%u cyls:%u",
				diskSize, drive.sects, drive.heads, drive.cyls);
			#endif
		}
		...
		drive.sects = 18;
		drive.heads = 2;
		if (diskSize <= 1228800) drive.sects = 15;
		if (diskSize <= 737280) drive.sects = 9;
		if (diskSize <= 368640) {
			drive.cyls = 40;
			drive.sects = 9;
		}
		...
		#ifdef DEBUG_DISK
		log(Log,"[DRIVEMANAGER::insertDisk] Detected Floppy Image size:%llu sects:%u heads:%u cyls:%u",
			diskSize, drive.sects, drive.heads, drive.cyls);
		#endif
	}
...
{
	#ifdef DEBUG_DISK
	log(Log,"[DRIVEMANAGER::ejectDisk] Disk Ejected for drive %u", drive);
	#endif
