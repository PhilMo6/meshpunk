  ...
  for (;;) {
    ...
    vmdispatch (GET_OPCODE(i)) {
      ...
      vmcase(OP_GETTABLE,
        ...
        // NO sandbox fallback here. A previous compat hack (mirroring the one
        // in OP_GETTABUP above) substituted the same-named cart global
        // whenever a field read came back nil. That breaks core Lua semantics
        // for every table read in every cart: `s.player==nil` sees the global
        // player CLASS, so prototype-OOP guards like `if (s.x==nil) s.x=...`
        // never run; celeste2's `self.collected` finds the cart's global
        // `collected` table; minified carts read functions out of numeric
        // fields. Field reads on a table must return nil on a miss; only
        // genuine global accesses (OP_GETTABUP through the _ENV upvalue) may
        // consult the sandbox. If an _ENV-override cart (e.g. Ex-Terra)
        // regresses, fix it by giving the override table an __index chain,
        // not by poisoning every table read in the VM.
      )
      vmcase(OP_SETTABUP,
        int a = GETARG_A(i);
