...
extern "C" uint32_t host_get_ticks_ms(void);
...
//extern "C" {
  ...
  #include <lauxlib.h>
  #include <fix32.h>
  #include <lstate.h>  // debug: string-table seed/occupancy (intern-split hunt)
//}

using namespace z8;

static void dump_string_table(lua_State* L, const char* tag);
        ...
        _currentCartdataKey("")
{
    ...
    //seed rng
    api_srand(fix32::frombits((int32_t)host_get_ticks_ms()));
    ...
    // dump_string_table(_luaState, "newstate"); // hunt telemetry — quiet for release; carterr path still dumps
...
Vm::~Vm(){
    ...
    if (_ownInput && _input != nullptr) {
        delete _input;
    }
    if (_ownGraphics && _graphics != nullptr) {
        delete _graphics;
    }
    if (_ownAudio && _audio != nullptr) {
        delete _audio;
    }
    if (_ownMemory && _memory != nullptr) {
        delete _memory;
    }
}
...
// Debug telemetry for the string-intern-split hunt: the hash seed must never
// change after lua_newstate; if it does, every later intern hashes into the
// wrong bucket and duplicate TStrings split table keys exactly like the
// nil-field cart crashes. size/nuse bracket string-table resize activity.
static void dump_string_table(lua_State* L, const char* tag) {
    global_State* g = G(L);
    printf("[luastr] %s seed=%08x strt_size=%d nuse=%d\n",
           tag, (unsigned)g->seed, g->strt.size, (int)g->strt.nuse);
}

// Debug disassembler for the cart-corruption hunt: the parser provably
// resolves `s.player` correctly (parse trace) yet runtime reads return the
// global — so dump the actual emitted constants + instructions of a cart
// function to see whether codegen or the VM is at fault.
#include "lopcodes.h"
static void dump_cart_fn(lua_State* L, const char* tbl, const char* fn) {
    lua_getglobal(L, "__cart_sandbox");
    if (!lua_istable(L, -1)) { lua_pop(L, 1); printf("[dump] no sandbox\n"); return; }
    lua_getfield(L, -1, tbl);
    if (!lua_istable(L, -1)) { lua_pop(L, 2); printf("[dump] no table %s\n", tbl); return; }
    lua_getfield(L, -1, fn);
    if (!lua_isfunction(L, -1)) { lua_pop(L, 3); printf("[dump] no fn %s.%s\n", tbl, fn); return; }
    if (lua_iscfunction(L, -1)) { lua_pop(L, 3); printf("[dump] %s.%s is a C function\n", tbl, fn); return; }
    const Closure* cl = (const Closure*)lua_topointer(L, -1);
    lua_pop(L, 3);
    const Proto* p = cl->l.p;
    printf("[dump] %s.%s linedef=%d params=%d upvals=%d nk=%d ncode=%d\n",
           tbl, fn, p->linedefined, (int)p->numparams,
           (int)p->sizeupvalues, p->sizek, p->sizecode);
    for (int i = 0; i < p->sizeupvalues; i++)
        printf("[dump] up%d '%s' instack=%d idx=%d\n", i,
               p->upvalues[i].name ? getstr(p->upvalues[i].name) : "?",
               (int)p->upvalues[i].instack, (int)p->upvalues[i].idx);
    for (int i = 0; i < p->sizek; i++) {
        const TValue* k = &p->k[i];
        if (ttisstring(k)) printf("[dump] K%d='%s'\n", i, svalue(k));
        else printf("[dump] K%d (type %d)\n", i, ttypenv(k));
    }
    for (int i = 0; i < p->sizecode; i++) {
        Instruction ins = p->code[i];
        OpCode o = GET_OPCODE(ins);
        printf("[dump] %3d L%-4d %-10s A=%d B=%d C=%d Bx=%d\n",
               i, p->lineinfo ? p->lineinfo[i] : -1,
               ((unsigned)o < NUM_OPCODES) ? luaP_opnames[o] : "BAD-OP",
               GETARG_A(ins), GETARG_B(ins), GETARG_C(ins), GETARG_Bx(ins));
    }
}

// Debug aid for cart faults: print the source the VM actually loaded around
// the failing line, so cart-decode or parse corruption is directly visible in
// the serial log next to the error. The error message looks like
//   [string "<first chars of chunk>..."]:LINE: <text>
// Only dumps when the errored chunk is the main cart chunk (carts can load()
// sub-chunks whose line numbers would not map to LuaString).
static void dump_error_context(const char* message, Cart* cart) {
    if (!message || !cart || cart->LuaString.empty()) return;

    const char* rb = strchr(message, ']');
    if (!rb || rb[1] != ':') return;
    int line = atoi(rb + 2);
    if (line <= 0) return;

    const std::string& src = cart->LuaString;

    // Verify the chunk-name prefix matches the start of the cart source
    const char* q1 = strchr(message, '"');
    if (q1 && q1 < rb) {
        const char* q2 = (const char*)memchr(q1 + 1, '"', rb - q1 - 1);
        if (q2) {
            size_t plen = (size_t)(q2 - q1 - 1);
            std::string prefix(q1 + 1, plen);
            if (plen >= 3 && prefix.compare(plen - 3, 3, "...") == 0)
                prefix.resize(plen - 3);
            if (!prefix.empty() && src.compare(0, prefix.size(), prefix) != 0) {
                printf("[cartsrc] error in non-main chunk, no source context\n");
                return;
            }
        }
    }

    int from = line > 3 ? line - 3 : 1;
    int cur = 1;
    size_t pos = 0;
    while (cur < from) {
        size_t nl = src.find('\n', pos);
    ...
    for (; cur <= line + 1; ++cur) {
        size_t nl = src.find('\n', pos);
        std::string ln = src.substr(pos, nl == std::string::npos ? std::string::npos
                                                                 : nl - pos);
        printf("[cartsrc] %4d%c %s\n", cur, cur == line ? '>' : ':', ln.c_str());
...
bool Vm::Step(){
    ...
    if (status != LUA_OK)
    {
        ...
        dump_error_context(message, _loadedCart);
    ...
    else
    {
        ...
        // Cart errors are caught Lua-side by __z8_tick (coroutine death) —
        // the message is stashed in __z8_last_error; dump source context
        // around the failing line before the cart change wipes state.
        if (tickResult == -1) {
            lua_getglobal(_luaState, "__z8_last_error");
            if (lua_isstring(_luaState, -1)) {
                const char* lerr = lua_tostring(_luaState, -1);
                printf("[carterr] %s\n", lerr);
                dump_error_context(lerr, _loadedCart);
                dump_string_table(_luaState, "carterr");
            }
            lua_pop(_luaState, 1);
            lua_pushnil(_luaState);
            lua_setglobal(_luaState, "__z8_last_error");
        }
    }
    lua_pop(_luaState, 1);

    // Corruption-hunt disassembler — root cause found (OP_GETTABLE sandbox
    // fallback in lvm.c); keep the tooling but leave it off.
    if (false) {
        dump_cart_fn(_luaState, "session", "reset_newgame");
    }
...
uint8_t Vm::vm_peek(int addr){
    // data[] is exactly MemoryUpperBound bytes, so addr == MemoryUpperBound
    // is already out of bounds (reachable via peek(0xffff, n) multi-reads)
...
void Vm::vm_poke(int addr, uint8_t value){
    // >= not >: addr == MemoryUpperBound would write 1 byte past data[]
    // (reachable via poke(0xffff, v1, v2) multi-pokes), corrupting whatever
    // the allocator placed after PicoRam.
    if (addr < 0 || addr >= MemoryUpperBound){
        return;
    }
...
void Vm::vm_poke2(int addr, int16_t value){
    if (addr < 0 || addr > MemoryUpperBound - 2){
        return;
    }
...
void Vm::vm_poke4(int addr, fix32 value){
    if (addr < 0 || addr > MemoryUpperBound - 4){
        return;
    }
...
void Vm::vm_reload(int destaddr, int sourceaddr, int len, string filename){
    ...
    if (sourceaddr + len > (int)sizeof(CartRomData)) {
        //source range runs past the cart rom (reload(0, 0x42ff, 0x4300)
        //would read ~17KB past the heap block otherwise)
        return;
    }
...
void Vm::vm_run() {
    ...
    // }
    // dump_string_table(_luaState, "vm_run"); // hunt telemetry — quiet for release
...
std::string Vm::getLuaLine(string filename, int linenumber) {
    ...
    // Walk through newlines to find the requested line number
    size_t pos = 0;
    while (linenumber > 0 && pos < luastr.size()) {
        size_t nl = luastr.find('\n', pos);
        if (nl == std::string::npos) { pos = luastr.size(); break; }
        pos = nl + 1;
        linenumber--;
    }
    ...
    std::string line;
    if (pos < luastr.size()) {
        size_t end = luastr.find('\n', pos);
        if (end == std::string::npos) end = luastr.size();
        line = luastr.substr(pos, end - pos);
    }
