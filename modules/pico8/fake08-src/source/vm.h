...
class Vm {
    ...
    // Per-dependency ownership: the Vm deletes only objects it created
    // itself. The old single shared flag deleted *injected* dependencies
    // too — freeing caller-owned pointers (and crashing outright once the
    // T-Deck port handed in a PicoRam living inside a larger guarded
    // allocation).
    bool _ownMemory;
    bool _ownGraphics;
    bool _ownInput;
