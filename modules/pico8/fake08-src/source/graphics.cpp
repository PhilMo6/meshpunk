	...
	bool flip_y) 
{
	...
	// right clip
	// min(...,128): clip_xe can be poked past the screen (0x5f22)
	if (scr_x + scr_w > std::min((int)drawState.clip_xe, 128)) {
		int nclip = (scr_x + scr_w) - std::min((int)drawState.clip_xe, 128);
		scr_w -= nclip;
	}
	...
	// bottom clip
	// min(...,128): clip_ye can be poked past the screen (0x5f23)
	if (scr_y + scr_h > std::min((int)drawState.clip_ye, 128)) {
		int nclip = (scr_y + scr_h) - std::min((int)drawState.clip_ye, 128);
		scr_h -= nclip;
	}
		...
		for (int y = 0; y < scr_h; y++) {
			...
			if (!flip_x) {
				for (int x = 0; x < scr_w; x++) {
					...
					if (shiftedPixIndex < 0 || shiftedPixIndex > 127) {
						continue;
					}
...
void Graphics::_private_h_line(int x1, int x2, int y){
	...
	// clip() clamps to the screen, but carts can poke 0x5f22/0x5f23 past
	// 128 — this path does direct row-pointer math, so clamp at use or the
	// memset below lands up to ~8KB past the screen buffer.
	int clip_xe = std::min((int)drawState.clip_xe, 128);
	int clip_ye = std::min((int)drawState.clip_ye, 128);

	if (!(y >= drawState.clip_yb && y < clip_ye)) {
		return;
	}

	if ((x1 < drawState.clip_xb && x2 < drawState.clip_xb) ||
		(x1 > clip_xe && x2 > clip_xe)) {
			return;
	}
	...
	// degenerate clip (begin past end) — clamp() pins minx above maxx and
	// the memset length would go negative (≈4GB as size_t)
	if (minx > maxx) {
		return;
	}
...
void Graphics::_private_v_line (int y1, int y2, int x){
	...
	// same poked-clip hazard as _private_h_line: the fast path below writes
	// through COMBINED_IDX directly, so bound the clip end at the screen
	int clip_xe = std::min((int)drawState.clip_xe, 128);
	int clip_ye = std::min((int)drawState.clip_ye, 128);

	if (!(x >= drawState.clip_xb && x < clip_xe)) {
		return;
	}

	if ((y1 < drawState.clip_yb && y2 < drawState.clip_yb) ||
		(y1 > clip_ye && y2 > clip_ye)) {
			return;
	}
